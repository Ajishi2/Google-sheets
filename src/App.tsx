...
function App() {
      ...
      <div className="absolute top-full left-0 bg-white shadow-lg border border-gray-200 rounded-md min-w-48 z-50">
        {items.map((item, index) => (
          ...
          >
            ...
            <div className="flex items-center">
              {item.shortcut && (
                <span className="text-gray-500 text-xs ml-6">{item.shortcut}</span>
              )}
              {item.submenu && (
  ...
  return (
    <div className="flex flex-col h-screen bg-white text-gray-700">
        ...
        <div className="flex border-t border-gray-200 relative" ref={menuRef}>
          ...
          {Object.keys(menuItems).map((menuName) => (
            <div key={menuName} className="relative">
              ...
              {activeMenu === menuName && (
                <MenuDropdown items={menuItems[menuName as keyof typeof menuItems]} menuName={menuName} />
              )}
            </div>
          ))}
        </div>
      ...
      <div className="flex items-center border-t border-b border-gray-300 bg-white px-2 py-1">
        <div className="flex items-center mr-2">
        ...
        <div className="w-10 h-6 flex items-center justify-center bg-gray-100 border border-gray-300 text-xs font-medium text-gray-600">
          A1
        </div>
        <div className="ml-2 flex-1">
          <input
            type="text"
            className="w-full h-6 px-2 border border-gray-300 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
            placeholder="fx"
          />
        </div>
      </div>
